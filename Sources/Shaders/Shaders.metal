...
//  SooshViewer
//
//  Created by Reif Stanfield on 8/11/26.
//
#include <metal_stdlib>
using namespace metal;

[[stitchable]] half4 solidColor(float2 position, half4 color, half4 customColor) {
    return customColor;
}

/// A shader that generates multiple twisting and turning lines that cycle
/// through colors.
///
/// From Inferno (https://www.github.com/twostraws/Inferno), MIT licensed.
///
/// This shader calculates how far each pixel is from one of 10 lines. Each line
/// has its own undulating color and position based on various sine waves, so the
/// pixel's color is calculated by starting from black and adding in a little of
/// each line's color based on its distance.
///
/// **Resolved by name at runtime**, as `ShaderLibrary.sinebow`. Renaming this
/// function, or dropping `[[stitchable]]`, is a crash the first time the view
/// draws rather than a build error.
///
